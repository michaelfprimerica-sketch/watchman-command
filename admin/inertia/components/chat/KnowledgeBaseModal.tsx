 ...
 * by Watchman" message in the action column carries the same signal).
 */
...
export default function KnowledgeBaseModal({
  ...
  const [managedCollection, setManagedCollection] = useState('')
  ...
  const uploadMutation = useMutation({
    mutationFn: (file: File) => api.uploadDocument(file, uploadCollection),
  })
  ...
  const assignCollectionMutation = useMutation({
    mutationFn: ({ source, collection }: { source: string; collection: string | null }) =>
      api.updateFileCollection(source, collection),
    onSuccess: (data, variables) => {
      addNotification({ type: 'success', message: data?.message || 'Collection updated.' })
      setCollectionDrafts((current) => {
        const next = { ...current }
        delete next[variables.source]
        return next
      })
      refreshCollections()
    },
  ...
  const renameCollectionMutation = useMutation({
    mutationFn: () => api.renameKnowledgeCollection(managedCollection, renamedCollection),
    onSuccess: (data) => {
      addNotification({ type: 'success', message: data?.message || 'Collection renamed.' })
      setManagedCollection('')
  ...
  const removeCollectionMutation = useMutation({
    mutationFn: () => api.deleteKnowledgeCollection(managedCollection),
    onSuccess: (data) => {
  ...
  const handleConfirmSync = () => {
    openModal(
      ...
      >
        <p className="text-text-primary">
          This will scan Watchman Command's storage directories for new files and queue them for
      ...
      <div className="bg-surface-primary rounded-lg shadow-xl max-w-4xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="overflow-y-auto flex-1 p-6">
            ...
            <div className="p-6">
              ...
              <div className="mx-auto mt-4 max-w-md">
                ...
                <CollectionCombobox
                  ...
                  label="Collection for uploaded documents"
                  disabled={isUploading}
                  placeholder="Uncategorized or type a new collection"
                />
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-desert-stone">
                      When you upload documents to your Knowledge Base, Watchman Command processes
                      and embeds the content, making it directly accessible to {aiAssistantName}.
                      This allows {aiAssistantName} to reference your specific documents during
                      conversations, providing more accurate and personalized responses based on
                      your uploaded data.
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-desert-stone">
                      Watchman Command includes built-in Optical Character Recognition (OCR)
                      capabilities, allowing it to extract text from image-based documents such as
                      scanned PDFs or photos. This means that even if your documents are not in a
                      standard text format, Watchman Command can still process and embed their
                      content for AI access.
                    </p>
                  </div>
                </div>
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-desert-stone">
                      Watchman Command will automatically discover and extract content you save to
                      your Information Library (if installed), making it instantly available to{' '}
          ...
          <div className="my-12">
            ...
            <div className="mb-5 rounded-lg border border-border-subtle bg-surface-secondary p-4">
              <div className="flex flex-wrap items-end gap-3">
                <div className="min-w-[12rem] flex-1">
                  <label
                    htmlFor="managed-collection"
                    className="mb-1 block text-sm font-medium text-text-primary"
                  >
                    Manage collections
                  </label>
                  <select
                    id="managed-collection"
                    value={managedCollection}
                    onChange={(event) => {
                      setManagedCollection(event.target.value)
                      setRenamedCollection(event.target.value)
                    }}
                    className="w-full rounded-md border border-border-default bg-surface-primary px-3 py-2 text-sm text-text-primary focus:border-transparent focus:outline-none focus:ring-2 focus:ring-desert-green"
                  >
                ...
                <StyledButton
                  variant="secondary"
                  size="sm"
                  onClick={() => renameCollectionMutation.mutate()}
                  disabled={
                    !managedCollection ||
                    !renamedCollection.trim() ||
                    renameCollectionMutation.isPending ||
                ...
                <StyledButton
                  ...
                  size="sm"
                  icon="IconTrash"
                  onClick={() => removeCollectionMutation.mutate()}
                  disabled={
                    !managedCollection ||
                    renameCollectionMutation.isPending ||
                    removeCollectionMutation.isPending
                  }
                  loading={removeCollectionMutation.isPending}
                >
                  Remove
                </StyledButton>
              </div>
              <p className="mt-2 text-xs text-text-muted">
                Type a new name while assigning a file to create a collection. Removing a collection
                only unassigns its files; it never deletes content or embeddings.
              </p>
            </div>
            <StyledTable<KbFileGroup>
              ...
              columns={[
                ...
                },
                {
                  ...
                  render(record) {
                    if (record.bucket === 'admin_docs') {
                      return <span className="text-sm text-text-muted">Watchman managed</span>
                    }
                    const draft =
                      collectionDrafts[record.source] === undefined
                        ? (record.collection ?? '')
                        : collectionDrafts[record.source]
                    const isPending =
                      assignCollectionMutation.isPending &&
                      assignCollectionMutation.variables?.source === record.source
                    return (
                      <div className="flex min-w-[14rem] items-center gap-2">
                        <CollectionCombobox
                          ...
                          collections={collections}
                          label={`Collection for ${record.displayName}`}
                          disabled={assignCollectionMutation.isPending}
                        />
                        <StyledButton
                          variant="secondary"
                          size="sm"
                          onClick={() =>
                            assignCollectionMutation.mutate({
                              source: record.source,
                              collection: draft.trim() || null,
                            })
                          }
                          disabled={
                            assignCollectionMutation.isPending ||
                            draft.trim() === (record.collection ?? '')
                          }
                          loading={isPending}
                        >
                          Apply
                ...
                },
                {
                  ...
                  title: '',
                  render(record) {
                    // Admin docs are auto-discovered and managed by Watchman Command —
        ...
        >
          <div className="text-text-primary text-sm space-y-3 text-left">
            <p>
              ...
              . For each file, Watchman Command will replace the existing embeddings in Qdrant and
              queue a fresh embedding job using the current chunking and embedding model.
            </p>
