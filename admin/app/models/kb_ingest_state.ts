...
export default class KbIngestState extends BaseModel {
  ...
  static async getOrCreate(filePath: string, collection?: string | null): Promise<KbIngestState> {
  ...
  static async markIndexed(
    ...
    row.last_error = null
    if (collection !== undefined) row.collection = collection
    await row.save()
  }
